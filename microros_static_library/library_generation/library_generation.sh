...
##### Generate pico sdk headers #####
mkdir -p /pico_min/build
cp /project/microros_static_library/library_generation/pico_min_cmakelists.txt /pico_min/CMakeLists.txt
pushd /pico_min/build > /dev/null
    cmake ..
popd > /dev/null
